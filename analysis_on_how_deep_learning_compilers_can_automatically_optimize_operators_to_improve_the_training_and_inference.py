# -*- coding: utf-8 -*-
"""Analysis on how deep learning compilers can automatically optimize operators to improve the training and inference.ipynb
...
def vgg16_model(input_shape=(28, 28, 1), num_classes=10):
...
def efficientnet_model(input_shape=(32, 32, 3), num_classes=10):
...
def plot_loss_accuracy(history, model_name):
    # Ensure the history is available
    if history is None:
...
def train_and_evaluate_model(dataset_name='mnist'):
    ...
    # Evaluate model performance and plot loss/accuracy
...
# Train, evaluate, and visualize for all datasets
for dataset in datasets:
...
# ... (Your existing code from the first two cells) ...
...
def tensorrt_infer(onnx_model_path, input_data):
    ...
    output_shape = (input_shape[0], 10)  # Adjust output shape if necessary

    d_input = cuda.mem_alloc(input_data.nbytes)
    d_output = cuda.mem_alloc(output_shape[0] * output_shape[1] * 4) # 4 bytes per float32
...
# Function to evaluate with TensorRT
...
# Call the evaluation function for desired datasets
evaluate_with_tensorrt(dataset_name='mnist')  # Example for MNIST